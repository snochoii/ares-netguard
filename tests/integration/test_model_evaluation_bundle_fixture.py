...
def test_fixture_generates_model_evaluation_bundle(tmp_path: Path) -> None:
    ...
    assert persisted["aggregate_summary"]["candidate_count"] == 8
    assert persisted["aggregate_summary"]["hypothesis_count"] >= 4
    assert persisted["aggregate_summary"]["score_row_count"] == 6
